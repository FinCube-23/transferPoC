...
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol"; // ReentrancyGuard from is in utils directory not in security directory
import "@openzeppelin/contracts/utils/Address.sol";
import "@openzeppelin/contracts-upgradeable/utils/ReentrancyGuardUpgradeable.sol"; // ReentrancyGuardUpgradeable is in utils directory not in security directory
    ...
    ReentrancyGuardUpgradeable
{
        ...
        returns (uint256 proposalId)
    {
        require(
            ...
            "targets/calldatas length mismatch"
        );
        require(values.length == targets.length, "values length mismatch");
        require(targets.length > 0, "empty proposal");
        require(targets[0] != address(0), "invalid target");
        require(calldatas[0].length > 0, "empty calldata");
        uint48 currentTime = uint48(block.timestamp);
        uint48 start = currentTime + uint48(votingDelay);
        uint48 end = start + uint48(votingPeriod);
        proposalId = proposalCount;
    ...
    function executeProposal(uint256 proposalId) external nonReentrant {
        ...
        if (proposalType[proposalId] == ProposalType.NewMemberProposal) {
            ...
            require(
                proposals[proposalId].target != address(0),
                "Invalid target"
            );
