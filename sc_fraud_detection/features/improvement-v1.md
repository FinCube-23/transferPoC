## Upload CSV.
1. CSV file should follow a specific format, not an issue as csv files are just plain text file so editable in any editor.
So inorder to make sure data-set quality the csv file to upload must have these columns
```txt
...
```
** if any data-set dont have the abobe mentioned fields than we will just use some default one(which means the insertion logic will change).

** if there are some extra fields then we will just ignore them.

- For now we will be using swagger for all these uploading related task, so add an warning field in swagger , if possible telling that result quality may decrease if dataset are not in proper format.
